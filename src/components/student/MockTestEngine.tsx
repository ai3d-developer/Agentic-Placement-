...
import { Award, Clock, CheckCircle2, Play, Sparkles, RefreshCw, FileText, ChevronRight, BarChart2, BookOpen, ExternalLink, Globe } from 'lucide-react';
...
export const MockTestEngine: React.FC = () => {
  ...
  // Study Resource Modal States
  const [resourceModalSkill, setResourceModalSkill] = useState<string | null>(null);
  const [resourceModalCategory, setResourceModalCategory] = useState<'Missing Skill (Gap Bridge)' | 'Provided Skill (Mastery Booster)'>('Provided Skill (Mastery Booster)');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {skillsList.length > 0 && (
          <button
            onClick={() => {
              setResourceModalSkill(skillsList[0]);
              setResourceModalCategory('Provided Skill (Mastery Booster)');
            }}
            className="px-4 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-500/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 text-xs font-bold flex items-center gap-2 transition shadow-sm cursor-pointer shrink-0"
          >
            <BookOpen className="w-4 h-4 text-indigo-500 group-hover:text-white" />
            <span>📚 Study Materials & Websites Hub</span>
          </button>
        )}
      </div>
      ...
      {!activeTest && !isGeneratingAI && (
        <>
          {profile.technicalSkills && profile.technicalSkills.length > 0 ? (
            <div className="p-4 rounded-2xl bg-emerald-50 text-emerald-800 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/20 text-xs font-bold flex items-center justify-between gap-2 flex-wrap">
              ...
              <span className="text-[10px] text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-500/20 px-2.5 py-1 rounded-lg">
                💡 Click "Study & Docs" on any card for free study materials & websites
              </span>
            </div>
          ) : (
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={isLocked}
                        className={`flex-1 py-2 px-3 rounded-xl font-bold text-[11px] flex items-center justify-center gap-1.5 transition-all border ${
                          isLocked
                      ...
                      <button
                        onClick={() => {
                          setResourceModalSkill(test.company);
                          setResourceModalCategory('Provided Skill (Mastery Booster)');
                        }}
                        title={`Study materials & websites for ${test.company}`}
                        className="py-2 px-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:text-indigo-600 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 text-[11px] font-bold flex items-center gap-1 transition cursor-pointer shrink-0"
                      >
                        <BookOpen className="w-3.5 h-3.5 text-indigo-500" />
                        <span className="hidden sm:inline">Study & Docs</span>
                      </button>
                    </div>
                  </div>
                </GlassCard>
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mt-1">
                {testResult.percentage >= 70
                  ...
                  : `A gap was detected in your ${testResult.company} fundamentals (${testResult.percentage}% achieved). Review the curated study materials and practice websites below to strengthen your score before applying for top-tier roles.`}
              </p>
            </div>

            <button
              onClick={() => {
                setResourceModalSkill(testResult.company);
                setResourceModalCategory(testResult.percentage >= 70 ? 'Provided Skill (Mastery Booster)' : 'Missing Skill (Gap Bridge)');
              }}
              className="w-full py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md shadow-indigo-600/20 transition cursor-pointer"
            >
              <BookOpen className="w-4 h-4" />
              <span>📚 Open {testResult.company} Study Materials, Videos & Recommended Websites</span>
            </button>
          </div>
        </GlassCard>
      )}
