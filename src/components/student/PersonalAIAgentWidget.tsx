...
import { Sparkles, Target, Briefcase, Award, Building, Clock, CheckCircle2, Bot } from 'lucide-react';
...
const INTRO_MESSAGE = "Hi! I'm your AI Agentic Placement Officer. 🎓";
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (i < INTRO_MESSAGE.length) {
        setTypedText(INTRO_MESSAGE.slice(0, i + 1));
        i++;
      } else {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-6 md:p-7 border border-indigo-100 dark:border-slate-800 shadow-xl mb-8">
      ...
      <div className="relative z-10 flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-gradient-to-r from-indigo-50 to-cyan-50 dark:from-indigo-950/50 dark:to-cyan-950/50 border border-indigo-100 dark:border-indigo-800/40">
          ...
          <p className="text-sm md:text-base font-extrabold text-indigo-700 dark:text-indigo-300 tracking-tight">
            {typedText}
            {!typingDone && (
              <span className="inline-block w-0.5 h-4 ml-0.5 bg-indigo-500 dark:bg-indigo-300 align-middle animate-pulse" />
            )}
          </p>
        </div>
